...
- **AIT ทั้ง 4 รอบ (หลัก + ซ้ำ 3) ตอบถูก 21/30 เท่ากันหมด และ F1 0.909 / 0.897 / 0.909 / 0.923** — ต่างจากตารางเดิมที่ได้ 21 กับ 28: **ภายใต้โค้ดปัจจุบัน ไม่พบรอบไหนกู้แถวที่ตราน้ำบัง (`90641004`, `90641008`) ได้เลย (0 จาก 4 รอบ)** ส่วนโค้ดเก่ากู้ได้ 2 จาก 4 รอบ ตรวจแล้วว่าแถวหายตั้งแต่ระดับ Markdown (Typhoon-OCR ไม่ได้อ่านออกมา) ไม่ใช่ขั้นแปลง JSON และโค้ดที่เตรียมภาพเข้าโมเดลไม่เปลี่ยนเมื่อปิดโหมดตัดตราน้ำ — รวมทุกรอบที่ตั้งค่านี้อ่านได้ 2 จาก 8 (โค้ดเก่า 2/4, ใหม่ 0/4) จึงแยกไม่ออกจากความแกว่งของ OCR (ตัวอย่างเล็ก) — หัวข้อ "AIT รัน 4 รอบ" ด้านบนจึงเป็นประวัติของโค้ดเก่า ห้ามอ้างเป็นพฤติกรรมปัจจุบัน; รอบทดลอง `AIT_dewm1–6` (ตัดตราน้ำ) ยังกู้สองแถวนี้ได้ แต่ไม่ได้ใช้เป็นผลหลักเพราะเลือกค่าตั้งรายเล่มตามผลประเมินคือการปรับให้พอดีชุดทดสอบ (ผลรอบนี้เก็บเป็นหลักฐานประกอบ)
...
หน้า "ปี 2 ภาคการศึกษาที่ 2" ของ IT coop ไม่เคยมีรหัส `06016419`/`06016420` ปรากฏในผล OCR เลยสักรอบเดียว (ตรวจย้อนทุกคอมมิต/ทุกรอบ retry ตั้งแต่ 2026-09-16) — เป็นจุดบอดถาวรของหน้านั้น ไม่ใช่ความแกว่งแบบสุ่ม ทดลองรัน `run_lab8b_it_coop_dewm.py` (ตัดตราน้ำ) 1 รอบ พบว่ากู้ทั้งสองรหัสได้ทันที (ยืนยันจาก `intermediate_vlm.md`/`pred_vlm.json`) F1 wildcard-aware ขยับ 0.788 → 0.849, CHK7F ปี 2/2 จาก 24 หน่วยกิต (เกิน) เป็น 18 (ตรงเล่ม) — **นำผลนี้มาแทนที่รันหลักของ `runs/IT/coop/` แล้ว** (ผลเดิมสำรองไว้ที่ `Lab8b_ocr_system/runs/IT/coop_pre_dewm_2026-09-22/`) รันฉันทามติชื่อ/รีเจนตัวเลข/verify/NL2SQL ใหม่ครบ

**ข้อควรระวัง:** `it_coop` (รันหลักใหม่) ใช้ค่าตั้ง `LAB7B_DEWATERMARK=1` แต่ `it_coop_retry` (ตาราง stability ด้านล่าง) ยังเป็นรอบเก่าที่ **ไม่ได้** ตัดตราน้ำ — คู่เทียบ "it_coop vs it_coop_retry" ด้านล่างจึงไม่ใช่การเทียบเงื่อนไขเดียวกันอีกต่อไป (ตัวเลขที่ผ่านเกณฑ์ 10% ในตารางนั้นจึง **ไม่ยืนยันความเสถียรจริง** สำหรับคู่นี้)
เป็นข้อยกเว้นเดียวในบรรดา 7 แผนหลัก — แผนอื่นทั้งหมดยังใช้ค่าตั้งเดียวกับรอบ retry ของตัวเอง
