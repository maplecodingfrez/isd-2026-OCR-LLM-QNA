...
แยกออกมาจาก `../Lab7B_Lab8B_ocr_system` โดยตั้งใจ — โค้ดในนี้ **อ่านผลลัพธ์ที่ Lab 8B รันไว้แล้วเท่านั้น**
...
4. **รู้จัก metric ที่วิชานี้ไม่ได้ใช้** — ตอบครบแล้ว ดูหัวข้อ "## 4." ที่ `evaluate_lab9.py`
   ...
   อื่น ๆ / LLM-as-a-judge ตัดสินใจไม่ใช้เพราะไม่มีงานที่ตรงเงื่อนไข; citation coverage/accuracy เดิมไม่ใช้
   แต่**ใช้แล้วตั้งแต่ 2026-09-26** เพราะคำตอบแนบหน้าอ้างอิงในเล่ม — ดูตารางแยกระดับคำถามในรายงาน)
...
| ฟิลด์ categorical (`ctype`) | confusion matrix, precision/recall/F1 ต่อ class, MCC | `Lab7B_Lab8B_ocr_system/.../lab7b_output/evaluation.json` (`alignment.classification.ctype`) | "งานจำแนก" — เผย accuracy paradox ที่ exact_match_acc มองไม่เห็น |
...
- **แก้ความไม่นิ่งด้วยการโหวตข้างมากข้ามรัน (สไลด์ ch9 หน้า 27) — ทดลองวัดผลแยก ไม่แทนที่ผลหลัก** (`Lab7B_Lab8B_ocr_system/experiments/ensemble_vote_2026-09-22/`): F1 ผลโหวต 0.898 เทียบค่าเฉลี่ยรายรัน 0.891, รันที่แย่สุด 0.871, รันที่ดีสุด 0.914; ดีกว่าค่าเฉลี่ยใน 4 จาก 7 แผน → **ลดกรณีแย่สุดได้บ้างแต่ไม่ชนะรันที่ดีสุดและไม่ดีขึ้นอย่างสม่ำเสมอ** จึงไม่นำมาใช้เป็นผลส่ง
