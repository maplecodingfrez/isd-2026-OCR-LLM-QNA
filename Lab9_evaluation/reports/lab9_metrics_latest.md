# Lab9 evaluation report — 2026-09-22 03:14
...
- **MAE** = 18.69 หน่วยกิต (เฉลี่ยจาก 16 run ที่มีข้อมูลครบ)
- **MAPE** = 14.76%
...
- **dsba_coop vs dsba_coop_retry**
  ...
  - Execution accuracy (SQL): dsba_coop=0.9333  vs  dsba_coop_retry=1.0  (ต่างกัน 6.7%) -> โอเค
  - ความถูกต้องของข้อความคำตอบ: dsba_coop=0.9333  vs  dsba_coop_retry=0.9  (ต่างกัน 3.6%) -> โอเค
...
- **dsba_no_coop vs dsba_no_coop_retry**
  ...
  - Execution accuracy (SQL): dsba_no_coop=0.8667  vs  dsba_no_coop_retry=0.9667  (ต่างกัน 10.3%) -> ⚠️ ไม่เสถียร (เกิน threshold)
...
- **it_no_coop vs it_no_coop_retry**
  - หน่วยกิตรวมในแผน: it_no_coop=120  vs  it_no_coop_retry=123  (ต่างกัน 2.4%) -> โอเค
  - จำนวนวิชาที่แปลงได้: it_no_coop=41  vs  it_no_coop_retry=42  (ต่างกัน 2.4%) -> โอเค
  - Execution accuracy (SQL): it_no_coop=0.7667  vs  it_no_coop_retry=0.8333  (ต่างกัน 8.0%) -> โอเค
  - ความถูกต้องของข้อความคำตอบ: it_no_coop=0.7667  vs  it_no_coop_retry=0.8333  (ต่างกัน 8.0%) -> โอเค
...
CHK1 เดิมบอกแค่ผ่าน/ไม่ผ่าน ไม่บอกขนาดความคลาดเคลื่อน — เพิ่ม MAE/MAPE ต่อยอด แล้ว (MAE=18.69 หน่วยกิต, MAPE=14.76% ข้ามทั้ง 7 หลักสูตร) เผยว่า run ที่ "fail CHK1" เหมือนกันหมด จริง ๆ คลาดเคลื่อนต่างกันมาก (dsba_no_coop แย่สุด 36 หน่วยกิต ขณะที่บาง run คลาดเคลื่อนแค่ ~5%)
