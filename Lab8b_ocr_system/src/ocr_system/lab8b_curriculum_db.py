...
def convert_lab7b(data: dict, *, program_id: str | None = None,
    ...
    for index, src in enumerate(data.get("courses") or []):
        ...
        # หมายเหตุ (2026-09-14): Lab 7B ไม่ใส่ฟิลด์ "prerequisite" ในผลลัพธ์อีกต่อไปแล้ว
        # (ดู COURSE_SCHEMA ใน lab7b_curriculum.py ส่วน "JSON SCHEMA") เพราะหน้าตารางแผนการศึกษา
        # ("ปีที่ N ภาคที่ M") ที่ป้อนเข้า pipeline นี้ไม่มีคอลัมน์วิชาบังคับก่อนอยู่เลย — ข้อมูลจริง
        # ถ้ามีอยู่ที่ภาคผนวก "3.4 คำอธิบายรายวิชา" คนละหน้าที่ไม่ได้ป้อนเข้ามา ตัดฟิลด์นี้ออกดีกว่า
        # ปล่อยให้โมเดลเดา (เคยเจอว่า qwen3:4b เดาความสัมพันธ์จากลำดับวิชาในตาราง ผิดชัดเจน)
        # บล็อกด้านล่างนี้จึงกลายเป็น no-op เสมอ (src.get("prerequisite") ได้ None ทุกครั้ง ->
        # pre_codes ว่างเปล่าเสมอ -> ไม่มีวัน insert อะไรเข้าตาราง prerequisite) — ปล่อยไว้แบบนี้
        # โดยตั้งใจ ไม่ลบทิ้ง เผื่ออนาคตมีคนป้อนหน้าที่มีข้อมูล prerequisite จริงเข้ามา (เช่น
        # ภาคผนวกคำอธิบายรายวิชา) จะได้ใช้ logic เดิมนี้ได้ทันทีโดยไม่ต้องเขียนใหม่
