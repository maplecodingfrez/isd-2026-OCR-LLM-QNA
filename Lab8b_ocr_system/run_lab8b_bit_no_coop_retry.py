"""รันซ้ำ BIT no-coop (เอกสารชุดเดียวกับ run_lab8b_bit_no_coop.py) เพื่อ "เช็คความเสถียร" — Lab9 checklist ข้อ 3

ทำอะไร: เอาภาพหน้าเดิม (runs/BIT/no_coop/data_input) ไปรัน Lab7B (OCR+LLM ใหม่จริง) -> Lab8B ใหม่ทั้งรอบ
แล้วเก็บผลไว้ที่ runs/BIT/no_coop_retry/ (แยกจาก runs/BIT/no_coop/ เดิม — ไม่เขียนทับ)
จากนั้น evaluate_lab9.py จะเทียบ bit_no_coop กับ bit_no_coop_retry ให้เอง ว่าผลแกว่งเกิน 10% ไหม
...
วิธีรัน (จากโฟลเดอร์ Lab8b_ocr_system):
    python run_lab8b_bit_no_coop_retry.py               # เต็มรอบ (ช้า — เรียกโมเดลจริง)
    python run_lab8b_bit_no_coop_retry.py --skip-lab7   # ใช้ pred_vlm.json ที่มีแล้ว รัน Lab8B ต่อ
...
RUN_DIR = ROOT / "runs" / "BIT" / "no_coop_retry"
SRC_INPUT = ROOT / "runs" / "BIT" / "no_coop" / "data_input"   # ภาพชุดเดิม (สำเนาไปใช้ ไม่แก้ต้นฉบับ)
