"""รัน BIT coop ใหม่โดยเปิด "ตัดตราน้ำก่อน OCR" (เฉพาะหน้าที่พบตราน้ำจริง) + "อ่านหน้าซ้ำเมื่อยอดรวมไม่ลงตัว"
(ผลทุกอย่างมาจาก OCR จริง ไม่แก้มือ) — ดัดแปลงจาก run_lab8b_bit_coop_retry.py / run_lab8b_ait_dewm.py

เอาภาพหน้าเดิม (runs/.../data_input) ไปรัน Lab7B ใหม่ -> Lab8B ใหม่ทั้งรอบ แล้วเก็บผลไว้ที่ runs/.../bit_coop_dewm/ (ไม่เขียนทับผลเดิม)
...
วิธีรัน (จากโฟลเดอร์ Lab8b_ocr_system):
    python run_lab8b_bit_coop_dewm.py               # เต็มรอบ (ช้า — เรียกโมเดลจริง)
    python run_lab8b_bit_coop_dewm.py --skip-lab7   # ใช้ pred_vlm.json ที่มีแล้ว รัน Lab8B ต่อ
"""
...
RUN_DIR = ROOT / "runs" / "BIT" / "coop_dewm"
