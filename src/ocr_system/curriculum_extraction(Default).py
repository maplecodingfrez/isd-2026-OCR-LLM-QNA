...
def _extract_page_courses(page: dict[str, Any], year: int | None, semester: int | None) -> list[dict[str, Any]]:
    # IMPORTANT: We now read words directly from page["text"] (a plain string
    # produced by pytesseract.image_to_string / paddle's line-joined text),
    # splitting on whitespace. The reading order is ALREADY correct because
    # the OCR engine reconstructed proper line/word order internally.
    #
    # We deliberately do NOT use page["lines"] + bounding-box (x, y) sorting
    # anymore. For Thai script, word-level box detection (image_to_data)
    # fragments text into individual glyphs with unreliable box coordinates
    # (vowels/tone marks sit above or below the baseline), so re-sorting by
    # (y, x) scrambles the reading order instead of fixing it.
    text = page.get("text", "")
    words = text.split()
 
    code_indexes = [index for index, word in enumerate(words) if CODE_RE.match(_clean_code(word))]
    courses = []
 
    for position, index in enumerate(code_indexes):
        code = _clean_code(words[index])
        next_index = code_indexes[position + 1] if position + 1 < len(code_indexes) else len(words)
        block = words[index + 1 : next_index]
        block = _trim_block(block)
        if not block:
...
def _course_from_block(code: str, block: list[str], year: int | None, semester: int | None) -> dict[str, Any]:
    texts = [text.strip() for text in block if text.strip()]
    credit_index = _first_credit_index(texts)
    credits = _normalize_credits(texts[credit_index]) if credit_index is not None else None
