...
- **IT coop เป็นข้อยกเว้นของแผนหลัก:** หน้าปี 2/2 ไม่เคยมีรหัส `06016419`/`06016420` ในผล OCR เลยสักรอบตั้งแต่วันแรก (จุดบอดถาวรของหน้านั้น ไม่ใช่ความแกว่งแบบสุ่ม — ตรวจย้อนทุกคอมมิต/ทุกรอบ retry ยืนยันแล้ว) ทดลองตัดตราน้ำ (`LAB7B_DEWATERMARK=1`) 1 รอบ กู้ได้ทันที F1 wildcard-aware 0.788 → **0.849**, CHK7F ปี 2/2 24→18 หน่วยกิต (ตรงเล่ม) — **นำมาแทนที่รันหลักของ `runs/IT/coop/` แล้ว** (ผลเดิมสำรองที่ `Lab8b_ocr_system/runs/IT/coop_pre_dewm_2026-09-22/`) เป็นแผนเดียวใน 7 แผนหลักที่เปิดตัดตราน้ำ ทำให้คู่เทียบ `it_coop` vs `it_coop_retry` ใน `Lab9_evaluation/README.md` ไม่ใช่เงื่อนไขเดียวกันอีกต่อไป — ดูรายละเอียด/คำเตือนที่นั่น
