"""Run the Lab 7B -> Lab 8B workflow on data/input_ait (หลักสูตร AIT — เทคโนโลยีปัญญาประดิษฐ์)

สำเนาจาก run_lab8b_dsba_coop.py (เดิมชื่อ run_lab8b_ours_coop.py ก่อนเปลี่ยนชื่อ 2026-09-16) ปรับให้:
- ใช้ data/input_ait (4 หน้า: ait_curriculum_page_023..026.jpg จาก outputs/ait/pages/ ของ
  Lab4-6 เดิม — คือหน้า "3.3 แผนการศึกษา" ทั้งเล่ม ปีที่ 1-4 เต็ม ไม่มีแยกสหกิจ/ไม่สหกิจเหมือน
  DSBA เพราะหลักสูตร AIT มีแผนเดียว — ยืนยัน page range ด้วยตาแล้ว ดู
  ISD/Learning Slides/Knowledge-based/lab9_progress.md)
- program-id "AIT", total_credits 120, years 4 (อ่านจากหน้า "รวมตลอดหลักสูตร 120 หน่วยกิต" ตรงๆ)
- ใช้ gold_questions.json ที่เตรียมไว้ล่วงหน้าแล้วที่ ../Lab9_evaluation/gold_questions/
  ait_gold_questions.json (คำนวณจาก ground truth คำนวณอัตโนมัติ ไม่ใช่เดามือ — ดู
  build_gold_questions.py) แทนที่จะรอ gold_questions_gt.json ที่ Lab7B สร้างเอง

วิธีรัน (จากโฟลเดอร์ Lab7B_Lab8B_ocr_system):
    python run_lab8b_ait.py               # เต็มรอบ: Lab7B OCR ใหม่ (เรียก Typhoon-OCR/qwen3 จริง) + Lab8B
    python run_lab8b_ait.py --skip-lab7   # ข้าม Lab7B ใช้ runs/AIT/lab7b_output/pred_vlm.json เดิม
...
def main() -> None:
    ...
    _book_txt = ROOT.parent / "outputs" / "ait" / "ait_curriculum_ocr.txt"
    if _book_txt.exists():
