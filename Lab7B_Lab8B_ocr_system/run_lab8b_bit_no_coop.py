"""Run the Lab 7B -> Lab 8B workflow on data/input_bit_no_coop (หลักสูตร BIT แผนไม่สหกิจ —
เทคโนโลยีสารสนเทศทางธุรกิจ (หลักสูตรนานาชาติ))

สำเนาจาก run_lab8b_ait.py ปรับให้:
- ใช้ data/input_bit_no_coop (5 หน้า: bit_curriculum_page_026..030.jpg จาก outputs/bit/pages/ ของ
  Lab4-6 เดิม — คือตาราง "แผนการศึกษา" แผนไม่สหกิจของ BIT ปีที่ 1-4 — ยืนยัน page range ด้วยตาแล้ว
...
- ใช้ gold_questions.json ที่เตรียมไว้ล่วงหน้าแล้วที่ ../Lab9_evaluation/gold_questions/
  bit_no_coop_gold_questions.json (คำนวณจาก ground truth คำนวณอัตโนมัติ ไม่ใช่เดามือ — ดู
  build_gold_questions.py) แทนที่จะรอ gold_questions_gt.json ที่ Lab7B สร้างเอง

วิธีรัน (จากโฟลเดอร์ Lab7B_Lab8B_ocr_system):
    python run_lab8b_bit_no_coop.py               # เต็มรอบ: Lab7B OCR ใหม่ (เรียก Typhoon-OCR/qwen3 จริง) + Lab8B
    python run_lab8b_bit_no_coop.py --skip-lab7   # ข้าม Lab7B ใช้ runs/BIT/no_coop/lab7b_output/pred_vlm.json เดิม
...
RUN_DIR = ROOT / "runs" / "BIT" / "no_coop"
LAB7_OUT = RUN_DIR / "lab7b_output"
LAB8_OUT = RUN_DIR / "lab8b_output"
GOLD_SRC = ROOT.parent / "Lab9_evaluation" / "gold_questions" / "bit_no_coop_gold_questions.json"
GT_SCOPED = ROOT.parent / "Lab9_evaluation" / "ground_truth_scoped" / "bit_no_coop_scoped.json"
...
def main() -> None:
    ...
    _book_txt = ROOT.parent / "outputs" / "bit" / "bit_curriculum_ocr.txt"
    if _book_txt.exists():
    ...
    # รหัสวิชาที่ OCR ทำหาย (ตราน้ำ / rowspan รวมสองแถว) -> กู้จากข้อความ OCR ทั้งเล่มด้วยกฎ (code_from_book.py)
    if _book_txt.exists():
        run(LAB7, "--recover-codes", prediction, "--book-ocr", _book_txt)
    ...
    book_ocr_txt = ROOT.parent / "outputs" / "bit" / "bit_curriculum_ocr.txt"
    ...
    book_ocr = ROOT.parent / "outputs" / "bit" / "bit_curriculum_ocr.txt"
    if book_ocr.exists():
    ...
    # ไม่มี OCR ทั้งเล่ม / Markdown / data_input -> คำสั่งพิมพ์บอกว่าข้าม (ไม่เงียบ, ไม่ทำให้ run ล้ม)
