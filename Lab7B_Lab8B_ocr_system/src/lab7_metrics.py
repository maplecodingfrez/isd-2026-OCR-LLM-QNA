...
# ==============================================================================
#  ส่วนที่ 8 — Classification metrics (confusion matrix / precision-recall / MCC)
#  ==========================================================================
#  ใช้กับฟิลด์ที่เป็นหมวดหมู่ปิด (ctype "บังคับ/เลือก", category "หมวดวิชา") ต่างจาก
#  FieldStat ด้านบนที่ออกแบบมาสำหรับ free text (name_th/name_en) ด้วย CER/WER —
#  ฟิลด์ categorical วัดด้วย exact_match_acc เพียงอย่างเดียวจะไม่เห็น "accuracy
#  paradox" (โมเดลเอนเอียงทาย class ที่มีจำนวนเยอะ ทำให้ accuracy รวมดูดีลวงตา
#  ทั้งที่ recall ของ class ส่วนน้อยแย่มาก) — confusion matrix + per-class P/R/F1
#  เผยปัญหานี้ได้ตรงกว่า (ดูสไลด์ ch9 ส่วนที่ 1 "งานจำแนก")
# ==============================================================================

_MISSING_LABEL = "<ไม่ได้ทำนาย>"


def classification_report(pairs: list[tuple]) -> dict:
    """
    pairs = [(gt_label, pred_label), ...] — pred_label เป็น None/"" ได้
    (กรณีวิชาที่โมเดลอ่านตกไปเลย ไม่มีค่าให้เทียบ)

    คืน confusion matrix + precision/recall/F1 ต่อ class + accuracy/macro F1
    รวมทั้งบล็อก + MCC เมื่อ GT มีพอดี 2 class จริง (งาน binary classification
    ตรงตามสูตรในสไลด์ ch9 — ไม่ใช่ 3+ class ที่ไม่มีนิยาม MCC มาตรฐาน)
    """
    from collections import defaultdict

    def norm(v: object) -> str:
        s = (str(v).strip() if v else "") or _MISSING_LABEL
        return s

    matrix: dict[str, dict[str, int]] = defaultdict(lambda: defaultdict(int))
    for g, p in pairs:
        matrix[norm(g)][norm(p)] += 1

    classes = sorted({norm(g) for g, _ in pairs} |
                      {c for row in matrix.values() for c in row})
    support = {c: sum(matrix[c].values()) for c in classes if c in matrix}

    per_class: dict[str, dict] = {}
    for c in classes:
        tp = matrix.get(c, {}).get(c, 0)
        fn = sum(matrix.get(c, {}).get(o, 0) for o in classes if o != c)
        fp = sum(matrix.get(o, {}).get(c, 0) for o in classes if o != c)
        prec = tp / (tp + fp) if (tp + fp) else 0.0
        rec = tp / (tp + fn) if (tp + fn) else 0.0
        f1 = 2 * prec * rec / (prec + rec) if (prec + rec) else 0.0
        per_class[c] = {
            "support": support.get(c, 0), "tp": tp, "fp": fp, "fn": fn,
            "precision": round(prec, 4), "recall": round(rec, 4), "f1": round(f1, 4),
        }

    active = [v for v in per_class.values() if v["support"] > 0]
    macro_f1 = round(sum(v["f1"] for v in active) / len(active), 4) if active else 0.0
    total = sum(support.values())
    correct = sum(matrix[c].get(c, 0) for c in matrix)
    accuracy = round(correct / total, 4) if total else 0.0

    result = {
        "classes": classes,
        "confusion_matrix": {c: dict(matrix.get(c, {})) for c in classes},
        "per_class": per_class,
        "accuracy": accuracy,
        "macro_f1": macro_f1,
        "n": total,
    ...
    real_classes = [c for c in classes if c != _MISSING_LABEL]
    if len(real_classes) == 2:
        pos, neg = real_classes
        tp = matrix.get(pos, {}).get(pos, 0)
        tn = matrix.get(neg, {}).get(neg, 0)
        fp = matrix.get(neg, {}).get(pos, 0)
        fn = matrix.get(pos, {}).get(neg, 0)
        denom = ((tp + fp) * (tp + fn) * (tn + fp) * (tn + fn)) ** 0.5
        result["mcc"] = round((tp * tn - fp * fn) / denom, 4) if denom else 0.0
        result["positive_class"] = pos

    return result


def print_classification_report(report: dict, title: str) -> None:
    """พิมพ์ confusion matrix + per-class P/R/F1 (+ MCC ถ้ามี) ลง console"""
    print()
    print("=" * 96)
    ...
    print("-" * 96)
    print(f"{'class':<24} {'support':>8} {'precision':>10} {'recall':>8} {'f1':>8}")
    for c, st in report["per_class"].items():
        if st["support"] == 0:
            continue
        print(f"{c:<24} {st['support']:>8} {st['precision']:>10.3f} "
              f"{st['recall']:>8.3f} {st['f1']:>8.3f}")
    print("-" * 96)
    tail = f"  accuracy={report['accuracy']:.3f}  macro_f1={report['macro_f1']:.3f}"
    if "mcc" in report:
        tail += f"  MCC={report['mcc']:.3f}  (positive class = {report['positive_class']})"
    print(tail)
    print("=" * 96)
