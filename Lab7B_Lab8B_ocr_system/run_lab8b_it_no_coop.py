"""Run the Lab 7B -> Lab 8B workflow on data/input_it_no_coop (หลักสูตร IT แผนไม่สหกิจ —
เทคโนโลยีสารสนเทศ)

สำเนาจาก run_lab8b_bit_coop.py ปรับให้:
- ใช้ data/input_it_no_coop (7 หน้า: it_curriculum_page_031..037.jpg จาก outputs/it/pages/ ของ
  Lab4-6 เดิม — คือตาราง "แผนการศึกษา" แผนไม่สหกิจของ IT ปีที่ 1-4 — ยืนยัน page range ด้วยแล้ว
  ดู ISD/Learning Slides/Knowledge-based/lab9_progress.md)
- program-id "IT", total_credits 129, years 4 (อ่านจากหน้า "รวมตลอดหลักสูตร 129 หน่วยกิต" ตรงๆ)
- ใช้ gold_questions.json ที่เตรียมไว้ล่วงหน้าแล้วที่ ../Lab9_evaluation/gold_questions/
  it_no_coop_gold_questions.json (คำนวณจาก ground truth คำนวณอัตโนมัติ ไม่ใช่เดามือ — ดู
  build_gold_questions.py) แทนที่จะรอ gold_questions_gt.json ที่ Lab7B สร้างเอง

วิธีรัน (จากโฟลเดอร์ Lab7B_Lab8B_ocr_system):
    python run_lab8b_it_no_coop.py               # เต็มรอบ: Lab7B OCR ใหม่ (เรียก Typhoon-OCR/qwen3 จริง) + Lab8B
    python run_lab8b_it_no_coop.py --skip-lab7   # ข้าม Lab7B ใช้ runs/IT/no_coop/lab7b_output/pred_vlm.json เดิม
...
RUN_DIR = ROOT / "runs" / "IT" / "no_coop"
LAB7_OUT = RUN_DIR / "lab7b_output"
LAB8_OUT = RUN_DIR / "lab8b_output"
GOLD_SRC = ROOT.parent / "Lab9_evaluation" / "gold_questions" / "it_no_coop_gold_questions.json"
GT_SCOPED = ROOT.parent / "Lab9_evaluation" / "ground_truth_scoped" / "it_no_coop_scoped.json"
